...
export function ProfileScreen({ user, onSaved, onSignOut }: ProfileScreenProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      saveLearningPreferences(user.id, { goal, sessionLength })
      onSaved(updated)
    } catch (caught) {
    ...
    <main className="min-h-screen bg-app-grid px-4 py-8 sm:grid sm:place-items-center">
      <section className="cq-panel w-full max-w-3xl overflow-hidden">
        ...
        <form className="mt-8 grid gap-6 sm:grid-cols-2" onSubmit={submit}>
          ...
          <div className="sm:col-span-2"><p className="mb-3 text-xs text-ink-muted">Your optional goals and time preference are saved on this device for now. You can change them from your profile.</p><button className="button-primary w-full sm:w-auto" type="submit" disabled={busy}>{busy ? 'Saving profile…' : 'Begin the journey →'}</button></div>
        </form>
        </div>
      </section>
    </main>
