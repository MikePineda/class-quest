    ...
    <div className="min-h-screen bg-app-grid">
      <header className="border-b border-white/10 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-primary font-black text-background" aria-hidden="true">CQ</span><div><p className="font-black">ClassQuest</p><p className="text-xs text-ink-muted">Server hub</p></div></div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
          <section className="rounded-2xl border border-white/10 bg-surface p-5 shadow-xl sm:p-7">
...
function CreateServerForm({ onCreated }: { onCreated: (server: ServerSummary) => void }) {
  ...
  if (created) {
    return (
      <div className="rounded-xl border border-secondary/30 bg-secondary/8 p-5" role="status">
        <p className="text-sm font-extrabold uppercase tracking-[0.16em] text-secondary">Server creation started</p>
        ...
        <div className="mt-4 flex items-center justify-between rounded-lg bg-background/55 px-4 py-3"><span className="text-sm text-ink-muted">Join code</span><strong className="font-mono text-lg tracking-[0.18em] text-primary-soft">{created.join_code ?? 'Pending'}</strong></div>
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      <label className="block rounded-xl border border-dashed border-white/20 bg-background/35 p-5 text-center hover:border-secondary/40"><span className="block font-bold">Attach lecture files</span><span className="mt-1 block text-xs text-ink-muted">Up to 10 files, 10 MB each</span><input className="mt-3 block w-full text-sm text-ink-muted file:mr-3 file:rounded-lg file:border-0 file:bg-surface-highest file:px-3 file:py-2 file:font-bold file:text-ink" type="file" accept=".txt,.md,.pdf,.docx,.pptx" multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []))} /></label>
...
function JoinServerForm({ onJoined }: { onJoined: (server: ServerSummary) => void }) {
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      <label className="block"><span className="field-label">Join code</span><input className="field font-mono uppercase tracking-[0.2em]" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="ABC123" maxLength={32} required autoComplete="off" /></label>
...
function ServerCard({ server }: { server: ServerSummary }) {
  ...
  return (
    <article className="rounded-xl border border-white/10 bg-surface-high/70 p-4">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-ink-muted"><span>{server.world_count} world{server.world_count === 1 ? '' : 's'}</span>{server.join_code && <><span aria-hidden="true">·</span><span>Code <strong className="font-mono text-ink">{server.join_code}</strong></span></>}</div>
